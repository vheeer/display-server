...
module.exports = (req, res) => {
  ...
  let { firstUser } = require("./tools/getID")( req );
  let acc = firstUser;
  Promise
  .all([
    getData("select * from good where able=1 and user_id=(select id from user where acc='" + acc + "');"), 
    getData("select * from mess where user_id=(select id from user where acc='" + acc + "');"),
    getData("select * from case_img where able=1 and user_id=(select id from user where acc='" + acc + "');"),
    getData("select * from good_img where good_id in (select id from good where user_id =(select id from user where acc='" + acc + "')" + ") and able = 1"),
    getData("select * from service_img where able=1 and user_id=(select id from user where acc='" + acc + "');"),
    getData("select * from bannar_img where able=1 and user_id=(select id from user where acc='" + acc + "');"),
  ])
  .then(function([
    { rows: rows_good }, 
    { rows: rows_mess },
    ...
    console.log("rows_bannar_img", rows_bannar_img);
    ...
    let main_data = {};
        ...
        main_data.theBannar = [];
        main_data.theBannar[0] = {};
        main_data.theBannar[0].imgs = [];
        ...
        main_data.serviceDesc = rows_mess[0]["service_desc"];
        ...
        rows_good.map(function(item){
          ...
          item.goodImgList = [];
          rows_good_img.map(function({ id, good_id, url }){
            if(good_id == item.id)
              item.goodImgList.push({ url, id });
          });
          main_data.goods.push(item);
        });

        rows_case_img.map(function({ id, url }){
          main_data.theCase[0].imgs.push({id, url});
        });

        rows_service_img.map(function({ id, url }){
          main_data.theService[0].imgs.push({id, url});
        });

        rows_bannar_img.map(function({ id, url }){
          main_data.theBannar[0].imgs.push({id, url});
        });

        res.send({ mes: "success", main_data });
  })
